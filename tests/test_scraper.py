...
class TestWebsiteScraper:
    ...
    async def test_scrape_example_com(self, scraper, browser_config):
        ...
        assert len(found_keywords) > 0, f"None of the expected keywords {expected_keywords} found in content"
    ...
    async def test_multiple_urls_with_keywords(self, scraper, browser_config):
        ...
        for result in results:
            assert result.error is None or result.error == "", f"Error scraping {result.url}: {result.error}"
            assert len(result.text) > 0, f"No text content found for {result.url}"
            
            # Check for httpbin keyword in all results
            content = (result.text + result.html).lower()
            assert "httpbin" in content, f"'httpbin' keyword not found in {result.url}"
...
async def test_keyword_search_simple():
    ...
    assert len(found_keywords) > 0, f"Expected keywords {expected_keywords} not found"
